...
describe("verifyTask returns errors", () => {
  ...
  test("rejects when resourceType not 'Task'", () => {
    ...
    const returnedErrors = verifyTask(
      invalidTask as fhir.Task,
      DISPENSING_USER_SCOPE,
      "test_sds_user_id",
      "test_sds_role_id"
    )
  ...
  test("no errors for a valid Task", () => {
    expect(verifyTask(validReturnTask, DISPENSING_USER_SCOPE, "test_sds_user_id", "test_sds_role_id")).toHaveLength(0)
    expect(verifyTask(validWithdrawTask, DISPENSING_USER_SCOPE, "test_sds_user_id", "test_sds_role_id")).toHaveLength(0)
  })

  test("rejects a message when only prescribing user scope present", () => {
    const result = verifyTask(validReturnTask, PRESCRIBING_USER_SCOPE, "test_sds_user_id", "test_sds_role_id")
  ...
  test("rejects a message when only prescribing app scope present", () => {
    const result = verifyTask(validReturnTask, PRESCRIBING_APP_SCOPE, "test_sds_user_id", "test_sds_role_id")
    expect(result).toEqual([errors.createMissingScopeIssue("Dispensing")])
  })

  test("accepts a message when only dispensing user scope present", () => {
    const result = verifyTask(validReturnTask, DISPENSING_USER_SCOPE, "test_sds_user_id", "test_sds_role_id")
    expect(result).toEqual([])
  })

  test("rejects a message when only dispensing app scope present", () => {
    const result = verifyTask(validReturnTask, DISPENSING_APP_SCOPE, "test_sds_user_id", "test_sds_role_id")
    expect(result).toEqual([errors.createUserRestrictedOnlyScopeIssue("Dispensing")])
  })

  test("console warn when inconsistent accessToken and body SDS user unique ID in Return message", () => {
    verifyTask(validReturnTask, DISPENSING_USER_SCOPE, "test_sds_user_id", "555086415105")
    expect(console.warn).toHaveBeenCalled()
  })

  test("console warn when inconsistent accessToken and body SDS role profile ID in Return message", () => {
    verifyTask(validReturnTask, DISPENSING_USER_SCOPE, "3415870201", "test_sds_role_id")
    expect(console.warn).toHaveBeenCalled()
  })

  test("console warn when inconsistent accessToken and body SDS user unique ID in Withdraw message", () => {
    verifyTask(validWithdrawTask, DISPENSING_USER_SCOPE, "test_sds_user_id", "555086415105")
    expect(console.warn).toHaveBeenCalled()
  })

  test("console warn when inconsistent accessToken and body SDS role profile ID in Withdraw message", () => {
    verifyTask(validWithdrawTask, DISPENSING_USER_SCOPE, "3415870201", "test_sds_role_id")
    expect(console.warn).toHaveBeenCalled()
  })
