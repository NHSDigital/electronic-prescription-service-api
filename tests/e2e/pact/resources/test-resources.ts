...
export const convertSecondaryCareCommunityAcuteCases = convertExamples.filter(e => e.isSuccess).map(spec => [spec.description, spec.request, spec.response, spec.responseMatcher, spec.statusCode])
export const convertSecondaryCareCommunityRepeatDispensingCases = convertExamples.filter(e => e.isSuccess).map(spec => [spec.description, spec.request, spec.response, spec.responseMatcher, spec.statusCode])
export const convertSecondaryCareHomecareCases = convertExamples.filter(e => e.isSuccess).map(spec => [spec.description, spec.request, spec.response, spec.responseMatcher, spec.statusCode])
