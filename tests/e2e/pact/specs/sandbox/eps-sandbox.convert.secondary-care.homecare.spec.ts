...
jestpact.pactWith(
  {
    ...
    provider: `nhsd-apim-eps-sandbox+convert-secondarycare-homecare+${process.env.PACT_VERSION}`,
  ...
  async (provider: any) => {
    ...
    describe("convert sandbox e2e tests", () => {
      ...
      test.each(TestResources.convertSecondaryCareHomecareCases)("should be able to convert %s message to HL7V3", async (desc: string, request: Bundle, response: string, responseMatcher: string) => {
