...
function signPrescription(
  ...
  const prepareResponse = convertFhirMessageToSignedInfoMessage(prepareRequest, "SHA1", "RS1", logger)
...
function calculateDigestFromPrescriptionRoot(prescriptionRoot: hl7V3.ParentPrescription) {
  ...
  const digestFromPrescriptionBase64 = createParametersDigest(fragmentsToBeHashed, "SHA1", "RS1")
