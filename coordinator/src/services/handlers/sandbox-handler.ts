...
import {Hl7InteractionIdentifier} from "../../models/hl7-v3/hl7-v3-datatypes-codes"
import {OperationOutcome} from "../../models/fhir/fhir-resources"
...
export class SandboxHandler implements RequestHandler {
  ...
  async poll(path: string): Promise<SpineResponse<OperationOutcome>> {
    if (path === this.parentPrescriptionPollingId || path === this.cancellationPollingId) {
      const responseOperationOutcome: OperationOutcome = {
